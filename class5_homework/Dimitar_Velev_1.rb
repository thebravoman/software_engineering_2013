...
kraen_srok=DateTime.parse('Mon Sep 30 16:00:00 2013 +0300')
all=Hash.new

CSV.foreach(File.join(ARGV[0].to_s, "Evaluation 2013-2014 - Sheet2.csv")) do |row|
	next unless row[0].to_s.length == 1
		bulg = row[1].to_s.strip + " " + row[2].to_s.strip
	if row[3].nil?
		english=bulg
	else
		english=row[3].to_s
		all[english] = [bulg,0,0,0]
	end
end
...
all.each do |key,value|
	if (value[1] <= kraen_srok && value[2] <= kraen_srok)
		value[3]="1"
	else
		value[3]="0"
	end
