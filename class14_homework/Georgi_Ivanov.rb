...
class ChartDrawer
	...
	def column_chart csv_file
		...
		chart_system data
		chart_system_text data
		draw_columns data
		draw_column_names data
	end
	
	private	
	def chart_system data
		n = data[1].length
		n.times do |y|
			@drawer.line(200,0+(y*50),-100,0+(y*50))
		end
	end
	
	def chart_system_text data
		n = data[1].length
		n.times do |y|
			@drawer.text(y*100,-130,0+(y*50))
		end
	end
	...
	def draw_column_names data
		loop_months(data) do |n,months,color|
			...
			@drawer.rect 20,20,(months*40)+120,n*30
			@drawer.text data[0][n+1],(months*40)+120+40,n*30
		end
	end
