...
class ChartDrawer
	...
	def column_chart csv_file
		...
		draw_graph_lines data
		draw_columns data
		draw_column_names data
		draw_line_values data
	...
	def loop_months data
		...
		colors = ["blue","red","orange","pink","green","purple","silver","yellow"]
		months.times do |n|
	...
	def draw_column_names data
		loop_months(data) do |n,months,color|
			...
			@drawer.rect 10,10,(months*40)+100,n*41
			@drawer.text data[0][n+1],(months*40)+100+11,n*41
		end
	end
	...
	def draw_graph_lines data 
		loop_months(data) do |n,months|
			if (n+(99*n)) < 400
				@drawer.line months*41,n+(99*n),-100,n+(99*n)
			end
		end
	end
	...
	def draw_line_values data 
		loop_months(data) do |n|
			@drawer.text ((n+(99*n)).to_s),-100,n+(99*n)+2
		end	
	end
end
